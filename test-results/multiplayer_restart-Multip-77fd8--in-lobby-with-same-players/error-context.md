...
- Name: multiplayer_restart.spec.js >> Multiplayer Restart — New Game button restart in same lobby >> 1. Host restart flow — game restart puts room back in lobby with same players
- Location: multiplayer_restart.spec.js:5:3
...
```ts
  ...
  2   | 
  3   | test.describe('Multiplayer Restart — New Game button restart in same lobby', () => {
  4   | 
  5   |   test('1. Host restart flow — game restart puts room back in lobby with same players', async ({ request }) => {
  6   |     const createResp = await request.post(`/api/multiplayer/create`, {
  7   |       data: { mode: 'progressif', wordCount: 1, nickname: 'Alice' }
  8   |     });
  9   |     expect(createResp.ok()).toBe(true);
  10  |     const createData = await createResp.json();
  11  |     const roomCode = createData.roomCode;
  12  |     const aliceID = createData.playerID;
  13  |     const aliceToken = createData.token;
  14  | 
  15  |     const joinResp = await request.post(`/api/multiplayer/join`, {
